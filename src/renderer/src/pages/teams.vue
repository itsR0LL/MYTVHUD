<template>
  <div class="team-card-container">
    <TransitionGroup name="transform-in" appear>
      <div v-for="team in teams" :key="team.id">
        <DropdownMenu
          :open="openedContextTeamId === team.id"
          @update:open="
            (val) => {
              if (!val && openedContextTeamId === team.id) openedContextTeamId = null
            }
          "
        >
          ...
          <DropdownMenuContent>
            ...
            <DropdownMenuItem variant="destructive" @click="deleteTeam(team.id)"
              >删除</DropdownMenuItem
            >
          </DropdownMenuContent>
        </DropdownMenu>
  ...
  <Transition name="fade" mode="out-in">
    <div v-if="showCreateTeamForm" class="create-team-form-container">
      <div class="form-title">{{ t(isEditing ? 'teams.editTitle' : 'teams.createTitle') }}</div>
...
async function createTeam() {
  ...
  if (missing.length) {
    toast.warning(t('common.missingRequired'), {
      description: t('common.pleaseFill', { fields: missing.join(' / ') }),
      duration: 3500
    })
    return
  }
  ...
  try {
    ...
    toast.error(t('common.dbWriteFailed'), {
      description: error?.message ?? t('common.dbWriteFailed'),
      duration: 4000
    })
  }
}
...
async function deleteTeam(id: string | number) {
  try {
    ...
    if (ok) {
      ...
      toast.warning(t('common.deleteFailed'), {
        description: t('common.recordNotFound'),
        duration: 3000
      })
    }
  } catch (error: any) {
